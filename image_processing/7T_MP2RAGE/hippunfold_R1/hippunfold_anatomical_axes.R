...
extract_coords <- function(hemi, hemi.name, surf){
	...
	write_metric_gifti(x = AP.coords, gifti_fname = sprintf("%s/output_measures/7T_MP2RAGE/hippunfold_R1/hippunfold_hemi-%s_label-%s_stat-APaxis.shape.gii", project_dir, hemi.name, surf), hemisphere = hemi)
	write_metric_gifti(x = LR.coords, gifti_fname = sprintf("%s/output_measures/7T_MP2RAGE/hippunfold_R1/hippunfold_hemi-%s_label-%s_stat-LRaxis.shape.gii", project_dir, hemi.name, surf), hemisphere = hemi)
}
