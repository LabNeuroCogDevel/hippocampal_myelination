...
Exclude sessions with bad hippunfold outputs (low dice)

```{r}
hippocampal.dice <- read.csv("/Volumes/Hera/Projects/hippocampal_myelin/output_measures/7T_MP2RAGE/hippunfold_R1/quality_control/hippocampus_dice.csv")
hippocampal.dice <- merge(hippocampal.dice, participants.qc)
hippocampal.dice <- hippocampal.dice %>% group_by(subject_id, session_id) %>% do(average.dice = mean(.$dice)) %>% unnest(cols = "average.dice") #calculate average dice score across left and right hemisphere. mean dice = 0.8088 +- 0.042

participants.qc <- participants.qc %>% filter(subses != "sub-11801_ses-20220602") #remove 1 sessions with bad hippunfold outputs from the 244, remaining N = 243 from 151 participants
```
...
participants.final <- participants.final %>% filter(!(subject_id == "sub-11822" & session_id == "ses-20210412")) #remove 4 sessions that are repeat MRI from the 244, resulting in N = 239 sessions from 151 participants
```

Exclude subject with incorrect (repeat assign) lunaid
```{r}
participants.final <- participants.final %>% filter(!(subject_id == "sub-11748" & session_id == "ses-20190401")) #remove 1 sub/session from the 239, resulting in N = 238 sessions from 150 participants
```
...
table(participants.final$mp2rage.session_number) #84 participants with 1 scan, 44 participants with 2 scans, 22 participants with 3 scans
```
